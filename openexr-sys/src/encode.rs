...
pub const EXR_ENCODE_DATA_SAMPLE_COUNTS_ARE_INDIVIDUAL: u16 = 1 << 0;
...
pub const EXR_ENCODE_NON_IMAGE_DATA_AS_POINTERS: u16 = 1 << 1;
...
pub struct exr_encode_pipeline_t {
    ...
    /// to using [`exr_encoding_choose_default_routines()`][1]. If a custom pack routine
    /// is written, that is up to the implementor.
    ///
    /// Describes the channel information. This information is
    /// allocated dynamically during [`exr_encoding_initialize()`][2].
    ///
    /// [1]: exr_encoding_choose_default_routines
    /// [2]: exr_encoding_initialize
    ...
    pub encoding_user_data: *mut c_void,
    ...
    /// adopt the pointer and set it to `NULL` here. Be cognizant of any
    /// custom allocators.
    pub compressed_buffer: *mut c_void,
    ...
    /// initialization.
    pub convert_and_pack_fn: Option<unsafe extern "C" fn(*mut exr_encode_pipeline_t) -> exr_result_t>,
    ...
    pub compress_fn: Option<unsafe extern "C" fn(*mut exr_encode_pipeline_t) -> exr_result_t>,
    ...
    pub yield_until_ready_fn: Option<unsafe extern "C" fn(*mut exr_encode_pipeline_t) -> exr_result_t>,

    /// Function chosen to write chunk data to the context.
    ///
    /// This is allowed to be overridden, but probably is not necessary
    /// in most scenarios.
    pub write_fn: Option<unsafe extern "C" fn(exr_encode_pipeline_t) -> exr_result_t>,
...
extern "C" {
    ...
    /// call [`exr_encoding_choose_default_routines()`][1].
    ///
    /// [1]: exr_encoding_choose_default_routines
    ...
    pub fn exr_encoding_run(ctxt: exr_const_context_t, part_index: c_int, encode_pipe: exr_encode_pipeline_t) -> exr_result_t;
    ...
    pub fn exr_encoding_destroy(ctxt: exr_const_context_t, encode_pipe: *mut exr_encode_pipeline_t) -> exr_result_t;
}